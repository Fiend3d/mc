...
func viewHelp(m *model) string {
	...
	normalDocsData := [][]string{
		...
		{" Ctrl+Left/Right", " - Move the current tab to the left / right pane, following it."},
		{"", " An empty pane keeps its half of the screen; T, b or gg fill it again."},
		{" Shift+Left/Right", " - Copy the current tab to the left / right pane, keeping the focus."},
		{" Y / X", " - Copy / move to the opposite pane, with an editable destination."},
		{" w", " - Background tasks; c cancels the highlighted task."},
		...
		{" ` (backtick)", " - Enter Message mode. The message history can be viewed here."},
