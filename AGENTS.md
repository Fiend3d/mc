...
- `v` in Normal mode opens Vibe inside a Git working tree. Its tree is rooted at the repository, includes combined HEAD-to-working-file changes plus untracked files, and refreshes every two seconds through the runtime timer. `vibe_git.go` reads bounded Git data; `vibe.go` owns tree state and generation-guarded refreshes; `view_vibe.go` renders; `vibe_viewer.go` opens the current working file with F3, mapping deleted lines to where they were; it never opens baseline copies. Preserve row/expansion state on refresh and keep input isolated from panes. Vibe is read-only and respects `git = false`.
