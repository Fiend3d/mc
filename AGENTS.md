...
- `v` in Normal mode opens Vibe inside a Git working tree. Its tree is rooted at the repository, includes combined HEAD-to-working-file changes plus untracked files, and refreshes every two seconds through the runtime timer. `vibe_git.go` reads bounded Git data; `vibe.go` owns tree state and generation-guarded refreshes; `view_vibe.go` renders; `vibe_viewer.go` opens current lines or captured baseline blobs with F3. Preserve row/expansion state on refresh, keep input isolated from panes, and clean up historical viewer files. Vibe is read-only and respects `git = false`.
