...
- Tab switches panes; Ctrl+Left/Right move the current tab across panes and Shift+Left/Right copy it; Ctrl+J enters Jump; Shift+D compares the cursor file in each pane (ignoring marked selections); Y/X transfer to the opposite pane; w opens tasks; e opens the selection with the shell's default verb (`shellOpen`, ShellExecute off the UI loop).
...
- `v` in Normal mode opens Vibe inside a Git working tree. Its tree is rooted at the repository, includes combined HEAD-to-working-file changes plus untracked files, and refreshes every two seconds through the runtime timer. At 100 columns or wider, Vibe shows a compact tree on the left and a separately scrolling diff preview on the right; narrower terminals use the combined full-width tree. Tab switches keyboard focus and mouse scrolling follows the pointer. `vibe_git.go` reads bounded Git data; `vibe.go` owns tree state and generation-guarded refreshes; `view_vibe.go` renders; `vibe_viewer.go` opens the current working file with F3, mapping deleted lines to where they were; it never opens baseline copies. Preserve row/expansion state on refresh and keep input isolated from panes. Vibe is read-only and respects `git = false`.
